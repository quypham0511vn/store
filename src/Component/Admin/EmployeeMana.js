...
export default function EmployeeMana({ navigation }) {
  const [token, settoken] = useContext(tokenContext);
  const [arrayUser, setArrayUser] = useState([]);
  const [removeUser, setRemoveUser] = useState(0);
  useEffect(() => {
    fetch("http://192.168.43.153:8000/api/employee_mana", {
      method: "POST",
      headers: {
  ...
  const handleremoveUser=(id)=>{
    ...
    if(removeUser !== 0){
    fetch("http://192.168.43.153:8000/api/employee_mana_delete", {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#554458" }}>
              ...
              <ScrollView key={index}>
                <View style={styles.row} key={index}>
                  ...
                  <View style={{ flexDirection: "column", marginLeft: 40 }}>
                    <TouchableOpacity
                    onPress={()=>navigation.navigate("UpdateEmployeeScreen",{idUser:item.id,emailUser:item.email})}
                    >
                      <View
                        style={{
                          width: 35,
      ...
      <View style={styles.buttonAdd}>
       ...
       onPress={()=>navigation.navigate("AddEmployeeScreen")}
        >
          <Text style={styles.textAdd}>Add Employee</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
